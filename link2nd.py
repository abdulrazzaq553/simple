...
def daily(selected,df):
    if selected=='Overall':
        ...
        month=pd.DataFrame(df.groupby('tem')['Message'].agg('count')).reset_index()
        return month
    else:
         check=df[df['User']==selected]
         time_intervals = {
        ...
        '10PM': '10PM-11PM', '11PM': '11PM-12AM'
         ...
         month1=pd.DataFrame(check.groupby('tem')['Message'].agg('count')).sort_values('Message',ascending=False).reset_index()
         return month1
